...
<template>
  <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
    <div class="flex items-center gap-2 mb-6">
      ...
      <h3 class="text-2xl font-bold text-primary">Projets</h3>
    </div>
    ...
    <div class="space-y-4">
      <div v-for="project in projects" :key="project.id"
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div class="flex-1">
            <div class="flex items-center gap-3 mb-2">
              ...
              <span class="text-sm text-gray-500 ml-auto">
                Client : {{ project.client?.nom_entreprise || 'N/A' }}
              </span>
            </div>

            <!-- Détail du devis associé -->
            <div v-if="project.quote" class="text-sm text-gray-600 mt-1">
              ...
              <p class="text-xs text-gray-500 mt-1">
                ...
                <span class="ml-3 px-2 py-0.5 rounded-full text-xs font-medium"
                      :class="{
                        'bg-yellow-100 text-yellow-700': project.quote.statut === 'envoye',
                        'bg-green-100 text-green-700': project.quote.statut === 'valide',
                        'bg-gray-100 text-gray-600': project.quote.statut === 'en_attente',
                        'bg-red-100 text-red-700': project.quote.statut === 'refuse',
                        'bg-purple-100 text-purple-700': project.quote.statut === 'termine'
                      }">
                  {{ project.quote.statut.toUpperCase() }}
                </span>
              </p>
            </div>
          ...
          <!-- Actions : simple consultation -->
          <div class="flex items-center gap-2 self-end sm:self-center">
            <router-link :to="'/dashboard/admin/projects/' + project.id"
