...
<template>
  <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
    <div class="flex items-center justify-between mb-4">
      ...
      <button @click="async () => { await api.post('/notifications/mark-read'); loadNotifications(); }" class="text-sm text-primary-light hover:underline">Tout marquer lu</button>
    </div>

    <div v-if="notifications.length === 0" class="text-gray-400 text-center py-4">Aucune notification.</div>
    <div v-for="notif in notifications" :key="notif.id" class="border-b border-gray-100 py-3 flex items-start gap-3 cursor-pointer" :class="notif.lu ? 'opacity-60' : ''" @click="markAsRead(notif)">
