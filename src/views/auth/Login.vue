  ...
  <div class="max-w-md mx-auto mt-10 bg-white p-8 rounded-2xl shadow-lg border border-gray-100">
    ...
    <form @submit.prevent="handleLogin" class="space-y-4">
      <div>
        ...
        <input type="email" v-model="email" class="w-full border rounded-xl px-4 py-2 focus:ring-2 focus:ring-primary-light outline-none" required>
      </div>
      <div>
        <label class="block text-sm font-semibold text-gray-700 mb-1">Mot de passe</label>
        <div class="relative">
          <input :type="showPassword ? 'text' : 'password'" v-model="password" class="w-full border rounded-xl px-4 py-2 focus:ring-2 focus:ring-primary-light outline-none pr-10" required>
          <button type="button" @click="showPassword = !showPassword" class="absolute inset-y-0 right-0 flex items-center pr-3 text-gray-500 hover:text-primary-light">
    ...
    <p class="mt-4 text-center text-sm text-gray-500">
      Pas encore de compte ? <router-link to="/register" class="text-primary-light font-semibold hover:underline">S'inscrire</router-link>
    </p>
