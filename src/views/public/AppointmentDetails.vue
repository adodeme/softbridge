...
const loadAppointment = async () => {
    try {
        ...
        if (e.response) {
            ...
            error.value = 'Le serveur ne répond pas. Vérifiez que Laravel est bien lancé.';
        } else {
...
const downloadPdf = async () => {
    try {
        const response = await api.get(`/appointments/${route.params.id}/download`, {
            responseType: 'blob'
        });
        ...
        link.href = url;
        link.setAttribute('download', `rdv_${route.params.id}.pdf`);
