  ...
  try {
    ...
    if (error.response) {
        ...
        if (error.response.status === 422) {
            ...
            Swal.fire({ icon: 'error', title: errorTitle, text: errorMsg, confirmButtonColor: '#d33' });
        } else {
            Swal.fire({ icon: 'error', title: 'Oups ! Une erreur serveur est survenue', text: `Code ${error.response.status} : ${errorMsg}`, confirmButtonColor: '#d33' });
        }
    } else if (error.request) {
        Swal.fire({ icon: 'error', title: 'Erreur de connexion', text: 'Le serveur Backend est inaccessible.', confirmButtonColor: '#d33' });
    } else {
        Swal.fire({ icon: 'error', title: 'Oups !', text: 'Une erreur inattendue est survenue.', confirmButtonColor: '#d33' });
    }
  } finally {
  ...
  <div class="max-w-3xl mx-auto px-4 py-12">
    <div class="bg-white rounded-2xl shadow-lg p-8 md:p-12 border border-gray-100">
      ...
      <form @submit.prevent="submitAppointment" class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="col-span-1 md:col-span-2"><input type="email" v-model="form.email" placeholder="Email" class="w-full border rounded-xl px-4 py-3" required></div>
        <div class="col-span-1 md:col-span-2"><input type="text" v-model="form.telephone" placeholder="Téléphone" class="w-full border rounded-xl px-4 py-3" required></div>
        <div class="col-span-1 md:col-span-2"><h4 class="font-bold text-gray-700 text-sm mb-2 border-b pb-2">Date et heure</h4></div>
